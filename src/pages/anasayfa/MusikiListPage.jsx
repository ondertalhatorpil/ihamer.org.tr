...
import { musikiSchools } from "../../data/musikiSchools";

const MusikiListPage = () => {
  ...
  const filteredData = filterData(musikiSchools);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-gradient-to-r from-teal-700 to-teal-900 text-white py-12 px-4">
        <div className="container mx-auto max-w-6xl">
          <h1 className="text-3xl md:text-4xl font-bold mb-3">
            Musiki Programı Uygulayan Anadolu İmam Hatip Liseleri
          </h1>
          <p className="text-blue-100">
            Geleneksel Türk musikisi ve cami musikisi eğitimi veren okullar
          </p>
        </div>
      </header>
      ...
      <div className="container mx-auto max-w-6xl px-4 pb-12">
        ...
        <div className="hidden md:block">
          {/* Table Header - STİCKY */}
          <div className="sticky top-[0px] z-[9998] bg-teal-700 text-white p-4 font-bold shadow-md rounded-t-lg">
            ...
            {filteredData.map((item, idx) => (
              ...
              >
                ...
                <div className="text-gray-600">
                  {highlightText(item.pansiyon)}
                </div>
                <div className="text-gray-600">
                  {highlightText(item.ogrenci)}
                </div>
              </div>
            ))}
          </div>
          ...
          {filteredData.map((item, idx) => (
            ...
            >
              <div className="space-y-3">
                <div>
                  <span className="font-semibold text-blue-700">İl: </span>
                  <span className="text-gray-800">
                    {highlightText(item.il)}
                  </span>
                </div>
                <div>
                  <span className="font-semibold text-blue-700">İlçe: </span>
                  <span className="text-gray-800">
                    {highlightText(item.ilce)}
                  </span>
                </div>
                <div>
                  <span className="font-semibold text-blue-700">Okul: </span>
                  <span className="text-gray-800">
                    {highlightText(item.okul)}
                  </span>
                </div>
                <div>
                ...
                <div>
                  <span className="font-semibold text-blue-700">Öğrenci: </span>
                  <span className="text-gray-600">
                    {highlightText(item.ogrenci)}
                  </span>
                </div>
        ...
        <div className="mt-6 text-center text-gray-600">
          <p className="font-semibold">
            Toplam <span className="text-teal-700">{filteredData.length}</span>{" "}
            okul listelendi
          </p>
        </div>
      </div>
