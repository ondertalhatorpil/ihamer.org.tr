...
import { Search, Book, User, Calendar, Building2, FileText, GraduationCap, ExternalLink, Filter, X, ChevronDown, ChevronUp, ArrowLeft, Tag } from 'lucide-react';
...
const FilteredTezList = () => {
  ...
  useEffect(() => {
    import('./data/tez.json')
      .then(module => {
        ...
        // URL parametresine göre filtrele
        const filtered = applyUrlFilter(enrichedData);
  ...
  const applyUrlFilter = (data) => {
    let result = [...data];
    
    switch(filterType) {
      case 'year':
        result = result.filter(t => t['Yıl'] === filterValue);
        break;
      case 'university':
        result = result.filter(t => t['Üniversite'] === decodeURIComponent(filterValue));
        break;
      case 'institute':
        result = result.filter(t => t['Enstitü'] === decodeURIComponent(filterValue));
        break;
      case 'department':
        result = result.filter(t => t['Bölüm'] === decodeURIComponent(filterValue));
        break;
      case 'category':
        result = result.filter(t => t.category === filterValue);
        break;
      case 'type':
        result = result.filter(t => t['Tez Türü'] === filterValue);
        break;
    }
  ...
  useEffect(() => {
    let result = applyUrlFilter(theses);
  ...
  }, [searchTerm, filters, theses, filterType, filterValue]);

  const getTitle = () => {
    switch(filterType) {
      case 'year': return `${filterValue} Yılı Tezleri`;
      case 'university': return `${decodeURIComponent(filterValue)} - Tezler`;
      case 'institute': return `${decodeURIComponent(filterValue)} - Tezler`;
      case 'department': return `${decodeURIComponent(filterValue)} - Tezler`;
      case 'category': return `${filterValue} Kategorisi`;
      case 'type': return `${filterValue} Tezleri`;
      default: return 'Tez Listesi';
    }
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">{filteredTheses.length} tez bulundu</p>
                </div>
              </div>
            </div>
          </div>
                  ...
                  {selectedThesis === index && (
                    <div className="bg-gradient-to-br from-slate-50 to-amber-50/30 rounded-xl p-4 border border-slate-200 animate-fade-in">
                      <div className="mb-3">
                        ...
                        <p className="text-sm text-slate-700 leading-relaxed">{thesis['Özet (Türkçe)']}</p>
                      </div>
                      {thesis['Özet (İngilizce)'] && (
                        <div className="pt-3 border-t border-slate-200">
                          <p className="text-xs text-justify font-bold text-[#B38F65] mb-2">English Abstract</p>
