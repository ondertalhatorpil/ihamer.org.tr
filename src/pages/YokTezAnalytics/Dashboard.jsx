...
import { ArrowUpRight, ArrowRight, Activity, Users, Building2, School, BookOpen, Search, X, BarChart2 } from 'lucide-react';
import { 
...
const styles = `
  ...
  * { font-family: "Lexend", sans-serif; box-sizing: border-box; }
  ...
  .list-row {
    ...
    color: var(--text-primary); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
    background: white; min-height: 72px; position: relative; overflow: hidden;
  }
  .list-row:hover { background-color: #F8FAFC; padding-left: 1.5rem; }
  .list-row.highlight-active { background-color: #fde047; border-color: #eab308; box-shadow: inset 0 0 0 1px #ca8a04; }
  .list-row:last-child { border-bottom: none; }

  .rank-circle { width: 28px; height: 28px; min-width: 28px; display: flex; align-items: center; justify-content: center; border-radius: 50%; font-size: 0.75rem; font-weight: 600; margin-right: 1rem; transition: all 0.2s; }
  ...
  .btn-primary { display: inline-flex; align-items: center; gap: 0.5rem; background: var(--text-primary); color: white; padding: 1rem 2rem; border-radius: 12px; font-weight: 500; transition: all 0.2s; border: 1px solid transparent; }
  ...
  .mini-search-input { width: 100%; padding: 6px 10px 6px 30px; font-size: 0.8rem; border: 1px solid #E5E7EB; border-radius: 6px; background: white; transition: all 0.2s; }
...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    // Araştırma yöntemi dağılımı (yeni alan)
    const methodCounts = allTheses.reduce((acc, t) => {
      const m = (t['Araştırmanın Yöntemi'] || '').trim();
      if (m) acc[m] = (acc[m] || 0) + 1;
      return acc;
    }, {});
    const methodData = Object.entries(methodCounts).sort((a,b)=>b[1]-a[1]).map(([name,value])=>({ name, value }));

    // Cinsiyet dağılımı (yeni alan)
    const genderCounts = allTheses.reduce((acc, t) => {
      const g = (t['Tez Yazarının Cinsiyeti'] || '').trim();
    ...
    const advisorMap = {};
    allTheses.forEach(t => {
      // Danışman artık unvan ayrılmış halde; tam isim için Danışman Unvanları + Danışman
      const unvan = (t['Danışman Unvanları'] || '').trim();
      const ad = (t['Danışman'] || '').trim();
      const fullName = unvan ? `${unvan} ${ad}` : ad;
      if (fullName.trim()) {
        if (!advisorMap[fullName]) advisorMap[fullName] = { name: fullName, count: 0, thesisIds: [], unvan };
        advisorMap[fullName].count += 1;
        advisorMap[fullName].thesisIds.push(t['Tez No']);
      }
    });
  ...
  useEffect(() => {
    ...
    if (advisorSearch.trim() === '') { setHighlightedAdvisorIndex(-1); return; }
  ...
  // Cinsiyet renkleri
  const genderColors = { 'Erkek': '#111827', 'Kadın': '#c7972f' };
  const methodColors = ['#111827', '#c7972f', '#6B7280', '#E5E7EB'];
        ...
        <div className="max-w-[1920px] mx-auto grid grid-cols-1 md:grid-cols-12 min-h-[500px]">
          ...
          <div className="col-span-1 md:col-span-6 grid grid-rows-2">
            <div className="border-b-soft p-8 md:p-12 flex flex-col justify-center reveal-up delay-200 bg-[#F9FAFB]">
              <span className="font-label text-gray-500 mb-3">Toplam Tez</span>
              <span className="font-display text-7xl md:text-8xl text-gray-900 tracking-tighter">
                <AnimatedNumber value={stats.total} />
              </span>
              {/* Araştırma yöntemi pills — sadece dolu veri varsa */}
              {stats.methodData.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {stats.methodData.map((m, i) => (
                    <span key={i} className="stat-pill" style={{ background: i === 0 ? '#111827' : i === 1 ? '#c7972f20' : '#F3F4F6', color: i === 0 ? 'white' : i === 1 ? '#c7972f' : '#6B7280' }}>
          ...
          <div className="lg:col-span-4 border-r-soft p-5 min-h-[400px] flex flex-col bg-white">
            ...
            <div className="flex-1 relative">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsPie>
                  <Pie data={stats.categoryData} cx="50%" cy="50%" innerRadius={80} outerRadius={110}
                    paddingAngle={5} dataKey="value" stroke="none" cornerRadius={8}
                    onClick={(data) => navigate(data.route)} className="cursor-pointer outline-none">
            ...
            <div className="flex gap-4 mt-4 justify-center">
              {stats.categoryData.map((d, i) => (
                <button key={i} onClick={() => navigate(d.route)}
                  className="flex items-center gap-2 text-sm font-medium hover:text-yellow-700 transition-colors">
                  <span className="w-3 h-3 rounded-full" style={{ background: i === 0 ? '#111827' : '#c7972f' }}></span>
                  {d.name} <span className="text-gray-400 font-normal">({d.value})</span>
                </button>
              ))}
            </div>
          </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.yearData}>
                  ...
                  <XAxis dataKey="year" axisLine={false} tickLine={false}
                    tick={{ fontFamily: 'Lexend', fontSize: 12, fill: '#9CA3AF' }} dy={10} />
                  <YAxis hide />
                  <Tooltip cursor={{ fill: '#F9FAFB' }} content={<CustomTooltip />} />
                  <Bar dataKey="count" fill="#E5E7EB" radius={[4,4,0,0]} activeBar={{ fill: '#c7972f' }}
          ...
          {[
            ...
            return (
              <div key={idx} className={`flex flex-col h-[700px] ${idx !== 3 ? 'xl:border-r-soft' : ''} ${idx % 2 === 0 ? 'md:border-r-soft' : ''} border-b-soft xl:border-b-0`}>
                <div className="p-5 border-b-soft backdrop-blur-sm sticky top-0 z-10 flex flex-col gap-3 bg-white">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-display text-lg flex items-center gap-2 text-yellow-900">
                        <Icon size={18} className="text-gray-400" /> {section.title}
                      </h4>
                    ...
                    <span className="bg-white border border-gray-200 text-xs font-semibold px-2 py-1 rounded-md text-gray-500">
                      Toplam {section.data.length}
                    </span>
                  </div>
                  {isAdvisorSection && (
                    <div className="relative w-full">
                      <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
                      <input type="text" placeholder="Danışman veya Tez No..."
                        value={advisorSearch} onChange={(e) => setAdvisorSearch(e.target.value)}
                        className="mini-search-input" />
                      {advisorSearch && (
                        <button onClick={() => setAdvisorSearch('')}
                ...
                <div className="overflow-y-auto flex-1 custom-scrollbar bg-white">
                  {section.data.map((item, i) => {
                    ...
                    return (
                      <a key={i} id={isAdvisorSection ? `advisor-row-${i}` : undefined}
                        href={linkPath} onClick={(e) => handleLinkClick(e, linkPath)}
                        className={`list-row group ${isHighlighted ? 'highlight-active' : ''}`}>
                        <div className={`rank-circle ${rankClass} ${isHighlighted ? 'bg-yellow-600 text-white' : ''}`}>{i + 1}</div>
                        <div className="flex-1 min-w-0 pr-3">
                          <span className={`text-sm truncate block pr-2 group-hover:text-black transition-colors ${isTop3 || isHighlighted ? 'font-semibold text-gray-900' : 'font-normal text-gray-600'}`}>
                            {item.name}
                          </span>
                          {isAdvisorSection ? (
      ...
      <footer className="border-b-soft py-24 bg-[#F9FAFB]">
        <div className="max-w-2xl mx-auto text-center px-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <button
              onClick={() => navigate('/tez-analytics/analiz')}
              className="btn-primary"
              style={{ background: '#c7972f', borderColor: '#c7972f' }}
            >
              Bibliyometrik Analiz <BarChart2 size={18} />
            </button>
          </div>
        </div>
      </footer>
