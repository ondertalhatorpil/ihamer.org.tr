...
const CyclingTitle = ({ isMobile }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', flexWrap: 'wrap', gap: '0.12em', lineHeight: 0.9 }}>
      {'İhamer'.split('').map((ch, ci) => (
        <motion.span key={ci}
          initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.65, delay: 0.2 + ci * 0.055, ease: [0.22, 1, 0.36, 1] }}
          style={{ display: 'inline-block', fontSize: fs, fontWeight: 800, lineHeight: 0.9, fontFamily: '"Playfair Display", serif', fontStyle: 'italic', color: T.navy, letterSpacing: '-0.03em' }}
        >{ch}</motion.span>
      ))}
      <div style={{ display: 'inline-block', overflow: 'hidden', height: isMobile ? 'clamp(44px,12vw,68px)' : 'clamp(60px,8vw,108px)', verticalAlign: 'bottom' }}>
        <AnimatePresence mode="wait">
          <motion.span key={phase}
            initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '-100%', opacity: 0 }}
            transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
            style={{ display: 'inline-block', fontSize: fs, fontWeight: 800, lineHeight: 1.2, fontFamily: '"Playfair Display", serif', fontStyle: 'italic', color: phase === 0 ? T.brown : T.navyMid, letterSpacing: '-0.03em', paddingLeft: '0.12em' }}
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: 16 }}>
            <Card delay={0.07} accent={T.navy}>
              ...
              <div style={{ marginTop: 16 }}>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={stats.topCities} margin={{ bottom: 52, left: -18 }}>
                    <CartesianGrid strokeDasharray="2 4" stroke={T.borderCard} vertical={false}/>
                    <XAxis dataKey="city" angle={-38} textAnchor="end" height={72} tick={{ fontSize: 10.5, fill: T.textMuted }} axisLine={false} tickLine={false}/>
            ...
            <Card delay={0.17} accent={T.brown}>
              ...
              <div style={{ marginTop: 16 }}>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={stats.topCityPct} margin={{ bottom: 52, left: -18 }}>
                    <CartesianGrid strokeDasharray="2 4" stroke={T.borderCard} vertical={false}/>
                    <XAxis dataKey="city" angle={-38} textAnchor="end" height={72} tick={{ fontSize: 10.5, fill: T.textMuted }} axisLine={false} tickLine={false}/>
                    <YAxis tick={{ fontSize: 10.5, fill: T.textMuted }} axisLine={false} tickLine={false}/>
                ...
                <p style={{ fontSize: 11, fontStyle: 'italic', color: T.textMuted, marginTop: 8 }}>* Kontenjan Sayısına Göre En Yüksek Oranlar</p>
              </div>
            </Card>
          </div>
        </div>
