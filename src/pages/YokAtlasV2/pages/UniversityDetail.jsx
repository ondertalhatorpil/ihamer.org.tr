...
/* ─────────────────────────────────────────────────────
   SCHOOL ROW — Universities.jsx'teki SchoolRow ile aynı
───────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────
   IHL SCHOOLS MODAL — Universities.jsx ile aynı tasarım
───────────────────────────────────────────────────── */
const SchoolModal = ({ univ, progName, data, onClose }) => {
  ...
  const schools = useMemo(() => {
    ...
    data
      ...
      .forEach(record => {
        ...
        const items = [];
        arr.forEach(item => {
          if (item.lise !== undefined) items.push(item);
          else if (Array.isArray(item.imam_hatip_liseler))
            item.imam_hatip_liseler.forEach(s => items.push(s));
        });
        items.forEach(s => {
          ...
          if (!schoolMap[nm]) schoolMap[nm] = { name: nm, city, count: 0 };
          schoolMap[nm].count += cnt;
        });
      });
    return Object.values(schoolMap).sort((a, b) => b.count - a.count);
  }, [data, univ.name, progName, year]);
  ...
  const totalStudents = schools.reduce((s, x) => s + x.count, 0);
  ...
  return (
    <>
      ...
      <div style={{ background: T.bg, minHeight: '100vh', padding: '0 0 80px',
        ...
        <section style={{
          padding: isMobile 
            ? 'clamp(28px,6vw,40px) clamp(16px,5vw,24px) clamp(24px,4vw,36px)'
            : 'clamp(36px,5vw,64px) clamp(20px,6vw,80px) clamp(28px,4vw,52px)',
          ...
          <div style={{ position: 'relative', zIndex: 2 }}>
            ...
            <motion.button onClick={() => navigate('/programs/v2')} whileHover={{ x: -3 }}
              style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none',
                cursor: 'pointer', color: T.textSub, fontSize: 12, marginBottom: isMobile ? 14 : 20, padding: 0,
            ...
            <motion.p
              ...
              style={{ fontSize: isMobile ? 9 : 10, fontWeight: 700, color: T.brown, textTransform: 'uppercase',
                letterSpacing: '0.2em', marginBottom: isMobile ? 10 : 14,
                display: 'flex', alignItems: 'center', gap: 8 }}
            >
              <span style={{ display: 'inline-block', width: isMobile ? 20 : 28, height: 1.5,
                background: T.brown, borderRadius: 1 }} />
              İHL Mezunları · Bölüm Detayı
            </motion.p>

            {/* Program adı */}
            <div style={{ overflow: 'hidden', marginBottom: isMobile ? 10 : 14 }}>
              <motion.span
                initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: 0.15, ease: [0.22, 1, 0.36, 1] }}
                style={{ display: 'inline-block',
                  fontSize: isMobile ? 'clamp(22px,7vw,32px)' : 'clamp(28px,5vw,68px)', 
                  fontWeight: 800, lineHeight: 1.1,
                  fontFamily: FONT_DISPLAY, fontStyle: 'italic',
                  color: T.navy, letterSpacing: '-0.02em' }}
              >{progData.name}</motion.span>
            </div>
            ...
            {progData.category && (
              <motion.div
                ...
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: isMobile ? 12 : 16,
            ...
            <motion.div
              ...
              style={{ height: 2, width: isMobile ? '70%' : 'min(45%, 280px)', originX: 0, marginBottom: isMobile ? 20 : 28,
                background: `linear-gradient(90deg, ${T.brown}, ${T.brownLight}77, transparent)`,
                borderRadius: 1 }}
            />

            {/* Stat strip */}
            <div style={{ display: 'flex', gap: isMobile ? 12 : 'clamp(16px,3vw,36px)', flexWrap: 'wrap' }}>
              {[
                ...
                >
                  <p style={{ fontSize: isMobile ? 9 : 10, color: T.textMuted, letterSpacing: '0.1em',
                    textTransform: 'uppercase', marginBottom: 4 }}>{s.l}</p>
                  <p style={{ fontSize: isMobile ? 'clamp(18px,5vw,24px)' : 'clamp(22px,3vw,32px)', fontWeight: 800, color: s.c,
              ...
              <motion.div
                ...
                style={{ borderLeft: `2px solid ${trend2425.direction === 'up' ? '#22a55e' : '#d94040'}33`, paddingLeft: isMobile ? 10 : 14 }}
              >
                <p style={{ fontSize: isMobile ? 9 : 10, color: T.textMuted, letterSpacing: '0.1em',
                  textTransform: 'uppercase', marginBottom: 4 }}>Trend</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  {trend2425.direction === 'up'
                    ? <TrendingUp size={isMobile ? 14 : 18} color="#22a55e"/>
                    : <TrendingDown size={isMobile ? 14 : 18} color="#d94040"/>}
                  <span style={{ fontSize: isMobile ? 'clamp(18px,5vw,24px)' : 'clamp(22px,3vw,32px)', fontWeight: 800, lineHeight: 1,
        ...
        {/* ── CONTENT ── */}
        <div style={{ padding: isMobile ? '20px 4vw 0' : '28px 5vw 0' }}>

          {/* ── İKİ KOLON ── */}
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(300px,1fr))', gap: 16, marginBottom: 16 }}>
            {/* 3 Yıl Tablosu */}
                  ...
                  <thead>
                    <tr>
                      {['Yıl','İHL','Toplam','Üniv.','Oran'].map(h => (
                        <th key={h} style={{ fontSize: isMobile ? 9 : 10, fontWeight: 700, color: T.textMuted,
                    ...
                    {progData.yearlyData.map((yd, i) => (
                      <tr key={yd.year}>
                        <td style={{ padding: '10px 0', borderBottom: `1px solid ${T.borderCard}`,
                          fontSize: i === 2 ? (isMobile ? 14 : 16) : (isMobile ? 12 : 13), fontWeight: i === 2 ? 800 : 600,
                          color: i === 2 ? T.navy : T.textSub, fontFamily: FONT_DISPLAY }}>{yd.year}</td>
                        <td style={{ textAlign: 'right', padding: '10px 0',
                          borderBottom: `1px solid ${T.borderCard}`, fontSize: isMobile ? 12 : 13, fontWeight: 700, color: T.text }}>
                          {yd.ihlCount.toLocaleString('tr-TR')}
                        </td>
                        <td style={{ textAlign: 'right', padding: '10px 0',
                          borderBottom: `1px solid ${T.borderCard}`, fontSize: isMobile ? 11 : 12, color: T.textSub }}>
                        ...
                        <td style={{ textAlign: 'right', padding: '10px 0',
                          borderBottom: `1px solid ${T.borderCard}`, fontSize: isMobile ? 12 : 13, fontWeight: 700, color: T.brown }}>
                          %{yd.percentage}
                        </td>
                      </tr>
                    ))}
            ...
            <Card accent={T.brown} delay={0.1}>
              ...
              <ResponsiveContainer width="100%" height={isMobile ? 160 : 180}>
                <LineChart data={progData.yearlyData}>
                  ...
                  <XAxis dataKey="year" tick={{ fontSize: isMobile ? 10 : 11, fill: T.textMuted }} axisLine={false} tickLine={false}/>
                  ...
                  <Line type="monotone" dataKey="ihlCount" stroke={T.brown} strokeWidth={2.5}
                    dot={{ fill: T.brown, r: isMobile ? 4 : 5, strokeWidth: 0 }}/>
                </LineChart>
              </ResponsiveContainer>
            </Card>
          </div>
          ...
          <Card accent={T.navy} delay={0.14} style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 16,
              marginTop: -10, padding: isMobile ? '6px 10px' : '8px 12px', background: T.brownPale, borderRadius: 8 }}>
              <Info size={12} color={T.brown}/>
              <span style={{ fontSize: isMobile ? 11 : 12, color: T.textSub }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 6 : 7 }}>
              {progData.topUniversities.map((u, i) => (
                <UnivRow key={i} univ={u} index={i} onIHLClick={handleUnivClick} navigate={navigate} isMobile={isMobile}/>
              ))}
            </div>
          </Card>
          ...
          <Card accent={T.brown} delay={0.18}>
            ...
            <div style={{ display: 'flex', alignItems: isMobile ? 'stretch' : 'flex-start',
              justifyContent: 'space-between', flexDirection: isMobile ? 'column' : 'row', gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ position: 'relative', flexShrink: 0 }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    width: isMobile ? '100%' : 'clamp(180px,25vw,300px)', transition: 'border-color 0.18s',
                  }}
                  onFocus={e => e.target.style.borderColor = T.brown}
                  onBlur={e  => e.target.style.borderColor = T.border}
                />
            ...
            <div style={{ overflowX: 'auto', overflowY: 'auto', maxHeight: '68vh' }}>
              <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, minWidth: isMobile ? 500 : 'auto' }}>
                <thead>
                  <tr>
                    {['Üniversite','Şehir','Tip','2025','Oran','Trend','İHL Detay'].map(h => (
                      <th key={h} style={{
                        fontSize: isMobile ? 9 : 10, fontWeight: 700, color: T.textMuted,
                        letterSpacing: '0.08em', textTransform: 'uppercase',
                        textAlign: h === 'Üniversite' ? 'left' : 'center',
                        padding: isMobile ? '8px 6px' : '10px 10px', whiteSpace: 'nowrap',
                ...
                <tbody>
                  {filteredUnivs.map((u, i) => (
                    ...
                    >
                      <td style={{ padding: isMobile ? '8px 6px' : '9px 10px', borderBottom: `1px solid ${T.borderCard}`,
                        fontSize: isMobile ? 11 : 13, fontWeight: 500, color: T.text }}>{u.name}</td>
                      <td style={{ textAlign: 'center', padding: isMobile ? '8px 6px' : '9px 10px',
                        borderBottom: `1px solid ${T.borderCard}`, fontSize: isMobile ? 10 : 12, color: T.textSub }}>
                        {u.city}
                      </td>
                      <td style={{ textAlign: 'center', padding: isMobile ? '8px 6px' : '9px 10px',
                      ...
                      <td style={{ textAlign: 'center', padding: isMobile ? '8px 6px' : '9px 10px',
                        borderBottom: `1px solid ${T.borderCard}`,
                        fontSize: isMobile ? 13 : 15, fontWeight: 800, color: T.navy, fontFamily: FONT_DISPLAY }}>
                        {u.count2025}
                      </td>
                      <td style={{ textAlign: 'center', padding: isMobile ? '8px 6px' : '9px 10px',
                        borderBottom: `1px solid ${T.borderCard}`,
                        fontSize: isMobile ? 10 : 12, fontWeight: 600, color: T.brown }}>
                        %{u.percentage2025}
                      </td>
                      <td style={{ textAlign: 'center', padding: isMobile ? '8px 6px' : '9px 10px',
                        borderBottom: `1px solid ${T.borderCard}` }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center',
                          ...
                          <button
                            ...
                            style={{ display: 'flex', alignItems: 'center', gap: 4,
                              padding: isMobile ? '3px 8px' : '4px 10px', background: T.brownPale, color: T.brown,
                          ...
                          >
                            <School size={isMobile ? 10 : 11}/> Liseler
                          </button>
                          <button
...
───────────────────────────────────────────────────── */
const UnivRow = ({ univ, index, onIHLClick, navigate, isMobile }) => {
  const [hov, setHov] = useState(false);
  return (
    <div
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      onClick={() => onIHLClick(univ)}
      style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 12, padding: isMobile ? '10px 12px' : '12px 14px',
        borderRadius: 11, cursor: 'pointer',
        background: hov ? T.bgDeep : 'transparent',
        border: `1px solid ${hov ? T.brown + '30' : T.borderCard}`,
        transition: 'all 0.15s' }}
    >
      <div style={{ width: isMobile ? 24 : 28, height: isMobile ? 24 : 28, borderRadius: 8, flexShrink: 0,
        background: index < 3 ? T.navy : `${T.navy}12`,
        display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontSize: isMobile ? 10 : 12, fontWeight: 700,
          color: index < 3 ? '#fff' : T.textMuted }}>{index + 1}</span>
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: isMobile ? 12 : 13, fontWeight: 500,
          color: hov ? T.brown : T.text,
          overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          transition: 'color 0.15s' }}>{univ.name}</p>
        <p style={{ fontSize: isMobile ? 10 : 11, color: T.textMuted,
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <p style={{ fontSize: isMobile ? 16 : 20, fontWeight: 800, color: T.navy,
          fontFamily: FONT_DISPLAY, lineHeight: 1 }}>{univ.count2025}</p>
        <p style={{ fontSize: isMobile ? 9 : 10.5, color: T.textMuted }}>%{univ.percentage2025}</p>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 3, flexShrink: 0, width: isMobile ? 44 : 52 }}>
