...
import { ArrowLeft, TrendingUp, TrendingDown, Building2, BookOpen, AlertCircle,
         ChevronRight, School, X, Search, GraduationCap, MapPin } from 'lucide-react';
...
/* Üniversite + bölüm kombinasyonu için lise listesi (tüm yıllar) */
const buildProgramSchools = (data, univName, progName) => {
  const schoolMap = {};
  data
    .filter(r =>
...
───────────────────────────────────────────────────── */
const SchoolModal = ({ univName, progName, schools, onClose }) => {
      ...
      >
        ...
        <div style={{ padding: '14px 24px 10px', borderBottom: `1px solid ${T.borderCard}`, flexShrink: 0 }}>
          ...
          <p style={{ fontSize: 11, color: T.textMuted, marginTop: 8 }}>
            ...
            {search ? ` (arama: "${search}")` : ' · tüm yıllar'}
          </p>
        </div>
        ...
        <div style={{ padding: '11px 24px 15px', borderTop: `1px solid ${T.borderCard}`,
          ...
          <p style={{ fontSize: 11, color: T.textMuted }}>
            Tüm yıllar (2023–2025) dahil · ESC veya dışına tıkla kapatmak için
          </p>
        </div>
      </motion.div>
    </motion.div>
...
const UniversityDetail = ({ data }) => {
  ...
  const [schoolModal, setSchoolModal] = useState(null); // { progName, schools }
  ...
  const openModal = (progName) => {
    const schools = buildProgramSchools(data, decodedName, progName);
    setSchoolModal({ progName, schools });
  };
          ...
          <div style={{ overflow: 'hidden', marginBottom: 14 }}>
            {uData.name.split('').slice(0, 20).map((ch, ci) => (
              <motion.span key={ci}
                ...
                style={{ display: 'inline-block', fontSize: 'clamp(24px,4vw,52px)', fontWeight: 800, lineHeight: 1.0, fontFamily: '"Playfair Display", serif', fontStyle: 'italic', color: T.navy, letterSpacing: '-0.02em' }}
              >{ch === ' ' ? '\u00A0' : ch}</motion.span>
            ))}
            {uData.name.length > 20 && (
              <motion.span
                ...
                style={{ display: 'inline-block', fontSize: 'clamp(24px,4vw,52px)', fontWeight: 800, lineHeight: 1.0, fontFamily: '"Playfair Display", serif', fontStyle: 'italic', color: T.brown, letterSpacing: '-0.02em' }}
